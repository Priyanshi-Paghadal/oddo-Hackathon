...
export const sendResetPasswordOTP = async (req, res) => {
  try {
    ...
    const userWithEmail = await User.findOne({ where: { email: emailLower } });
    ...
    try {
      ...
      await sendNotification(admin.id, notificationMessage);
...
export const verifyAdminLoginOTP = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      where: { email: emailLower, role: 'Admin', isActive: true }
    });
